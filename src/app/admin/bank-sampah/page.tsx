  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {loading ? (
        ...
        <div className="bg-slate-800/40 rounded-2xl border border-slate-700/50 py-12 text-center text-slate-600 text-sm">
        ...
        <>
          ...
          <div className="lg:hidden flex flex-col gap-3">
            {filtered.map(b => (
              <div key={b.id} className="bg-slate-800/40 rounded-2xl border border-slate-700/50 p-4">
                <div className="font-semibold text-white text-sm">{b.nama}</div>
                <div className="text-xs text-slate-500 mt-0.5">{b.kecamatan} · {b.wa || '–'}</div>
                ...
                <div className="flex gap-1.5 flex-wrap mt-3 pt-3 border-t border-slate-700/40">
                  ...
                  <Link href={`/bank-sampah/${b.slug}`} target="_blank"
                    className="px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-700/80 text-slate-300 hover:text-white border border-transparent transition-all inline-flex items-center gap-1.5">
                    <FiExternalLink className="w-3.5 h-3.5" /> Lihat
                  </Link>
                </div>
              </div>
            ))}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Kecamatan</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="px-5 py-3.5 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Daftar</th>
              ...
              <tbody className="divide-y divide-slate-700/30">
                {filtered.map(b => (
                  <tr key={b.id} className="hover:bg-slate-700/20 transition-colors group">
                    ...
                    <td className="px-5 py-4 text-slate-400 text-sm">{b.kecamatan}</td>
                    <td className="px-5 py-4">
