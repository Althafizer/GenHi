...
export default function StatsSection({ stats }: { stats: GlobalStats }) {
  const items = [
    { icon: '🏦', val: stats.total_bank_aktif || 47, suf: '', label: 'Bank Sampah Aktif', sub: 'di Kota Yogyakarta' },
    { icon: '♻️', val: stats.total_sampah_kg ? stats.total_sampah_kg / 1000 : 2.4, suf: ' Ton', label: 'Sampah Terkumpul', sub: 'per bulan, seluruh jaringan' },
    { icon: '🗺️', val: stats.total_kecamatan || 14, suf: '', label: 'Kecamatan', sub: 'tertangani di Yogyakarta' },
    { icon: '👥', val: stats.total_nasabah || 3200, suf: '+', label: 'Nasabah Aktif', sub: 'bergabung bersama GenHi' },
  ];
  return (
