export default function AboutSection() {
  const pillars = [
    { icon: '💻', title: 'Digital Platform', desc: 'Sistem digital yang menghubungkan nasabah dengan bank sampah terdekat secara real-time.' },
    { icon: '🤝', title: 'Social Empowerment', desc: 'Memberdayakan komunitas lokal melalui jaringan bank sampah yang solid dan transparan.' },
    { icon: '💰', title: 'Sustainable Economy', desc: 'Sampah menjadi sumber penghasilan nyata bagi masyarakat dan pengelola bank sampah.' },
  ];
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="relative">
            <div className="rounded-3xl overflow-hidden bg-gradient-to-br from-green-950 to-green-700 p-10 min-h-[400px] flex items-center justify-center relative">
              <div className="text-[120px] opacity-20 select-none">♻️</div>
              <div className="absolute inset-0 bg-[radial-gradient(rgba(63,201,109,0.07)_1px,transparent_1px)] bg-[size:30px_30px]" />
              <div className="absolute top-[12%] -right-4 bg-green-500 text-white text-sm font-bold px-4 py-2.5 rounded-2xl shadow-xl z-10">
                🌿 Eco Certified
              </div>
              <div className="absolute bottom-[14%] -left-4 bg-green-700 text-white text-sm font-bold px-4 py-2.5 rounded-2xl shadow-xl z-10">
                ♻️ 47 Bank Aktif
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              {pillars.map((p, i) => (
                <div key={i} className="flex gap-4 items-start p-4 rounded-2xl bg-green-50 border border-green-100">
                  <div className="w-11 h-11 bg-green-100 rounded-xl flex items-center justify-center text-xl shrink-0">{p.icon}</div>
                  <div>
