    ...
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        {linkError
          ...
          : <p className="text-xs text-white/30 mt-1">Buka tempat di Google Maps → Share → Salin link, lalu tempel di sini.</p>}
      </div>
      ...
      <div className="flex items-end gap-4">
        ...
        {lat != null && lng != null && (
          <button type="button" onClick={clearLocation}
            className="shrink-0 text-xs font-bold text-red-400 border border-red-500/30 px-4 py-3 rounded-xl hover:bg-red-500/10 transition-colors">
            🗑 Hapus lokasi
          </button>
        )}
      </div>
      ...
      {lat == null || lng == null ? (
        <p className="text-xs text-amber-400">⚠️ Lokasi belum diatur — bank sampahmu tidak akan muncul di peta & pencarian terdekat.</p>
      ) : (
        <p className="text-xs text-green-400">✓ Lokasi sudah ditandai. Jangan lupa klik &quot;Simpan Perubahan&quot;.</p>
      )}
    </div>
  );
}
